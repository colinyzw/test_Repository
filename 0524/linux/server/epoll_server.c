/****************************************************“
  ...
  > Modified Time:2022年05月02日 星期一 14时59分54秒
 ****************************************************/
...
int epoll_starting(int sockfd)
{
	...
	//文件描述符，默认情况下进程打开的最大文件描述符为1024

	printf("epoll server Running...\n");
	while(SHUTDOWN){
		readycode = epoll_wait(epfd,ready_array,EPOLLSIZE,-1);//阻塞监听sock读事件，就绪时返回
		while(readycode){//循环处理就绪事件
			i = 0;
			if(ready_array[i].data.fd == sockfd){//serverfd就绪
				...
				if((server_recv_response(ready_array[i].data.fd))==-1){
					close(ready_array[i].data.fd);//删除监听
					epoll_ctl(epfd,EPOLL_CTL_DEL,ready_array[i].data.fd,NULL);//删除数据
				}
			}
			--readycode;
			++i;
